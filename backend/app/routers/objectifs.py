...
def create_objectif(objectif: ObjectifCreate, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
...
def get_objectifs(db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
...
def get_objectif_by_id(
    objectif_id: int = Path(..., gt=0), db: Session = Depends(get_db), user: dict = Depends(get_current_user)
):
    objectif = db.query(Objectif).filter(Objectif.id_objectif == objectif_id).first()
...
def delete_objectif(objectif_id: int, db: Session = Depends(get_db), user: dict = Depends(require_admin)):
    objectif = db.query(Objectif).filter(Objectif.id_objectif == objectif_id).first()
