def test_create_exercice_admin(client, admin_headers):
    ...
    response = client.post("/exercices", json=data, headers=admin_headers)
...
def test_get_exercices(client, admin_headers):
    response = client.get("/exercices", headers=admin_headers)
...
def test_get_exercice_not_found(client, admin_headers):
    response = client.get("/exercices/999", headers=admin_headers)
